...
| **Tailscale Serve** | Tailscale on the computer (Linux or macOS) and the phone, with HTTPS certificates enabled on the tailnet. | Your tailnet | No third-party transport at all — the only external piece is Tailscale's coordination server. See [tailscale.md](tailscale.md). |
