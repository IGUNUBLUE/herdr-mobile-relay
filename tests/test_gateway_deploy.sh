...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/lerdr-gateway-deploy-test.XXXXXX")"
...
export LERDR_COMMUNITY_GATEWAY_URL="wss://community-a.example.test,wss://community-b.example.test"

HOSTNAME_UNDER_TEST="gw.example.test"
export LERDR_GATEWAY_DEPLOY_HOST="$HOSTNAME_UNDER_TEST"
export LERDR_GATEWAY_DEPLOY_EMAIL="ops@example.test"
...
export LERDR_GATEWAY_DEPLOY_DIR="$BUNDLE_DIR"
export LERDR_RELAY_ENV="$WORK_DIR/relay-bundle-only.env"
: > "$LERDR_RELAY_ENV"
run_deploy
...
for SOURCE in Dockerfile.gateway go.mod go.sum \
    cmd/lerdr-gateway/main.go internal/gateway/server.go \
...
grep -Fq 'context: ${LERDR_GATEWAY_BUILD_CONTEXT:-./gateway-source}' \
    "$BUNDLE_DIR/docker-compose.yml" || fail "compose file lost the bundled build context"
grep -Fq 'LERDR_GATEWAY_BUILD_CONTEXT="./gateway-source"' "$BUNDLE_DIR/.env" ||
...
grep -Fq "LERDR_GATEWAY_VERSION: \${LERDR_GATEWAY_VERSION:-$EXPECTED_VERSION}" \
    "$BUNDLE_DIR/docker-compose.yml" || fail "compose file does not pass the gateway release to the build"
grep -Fq "LERDR_GATEWAY_VERSION=$EXPECTED_VERSION" "$BUNDLE_DIR/.env" ||

    fail ".env does not record the deployed gateway release"
grep -Eq '^LERDR_GATEWAY_REVISION=[0-9a-f]{40}$' "$BUNDLE_DIR/.env" ||
    fail ".env does not record the deployed gateway revision"
grep -Fq 'ARG LERDR_GATEWAY_VERSION=dev' "$BUNDLE_DIR/gateway-source/Dockerfile.gateway" ||
...
grep -Fq 'LERDR_GATEWAY_STUN_ADDR' "$BUNDLE_DIR/.env" ||
...
grep -Fq 'LERDR_GATEWAY_STUN_ADDR: ${LERDR_GATEWAY_STUN_ADDR-:3478}' \
    "$BUNDLE_DIR/docker-compose.yml" ||
    fail "compose file does not default the address discovery listener on unset only"
! grep -Fq '${LERDR_GATEWAY_STUN_ADDR:-' "$BUNDLE_DIR/docker-compose.yml" ||
...
for KNOB in LERDR_GATEWAY_MAX_RELAYS LERDR_GATEWAY_MAX_CLIENTS; do
...
if grep -q '^LERDR_GATEWAY_URL=' "$LERDR_RELAY_ENV"; then
...
export LERDR_GATEWAY_DEPLOY_DIR="$BUNDLE_DIR"
export LERDR_GATEWAY_DEPLOY_SERVER="deploy@gw.example.test"
export LERDR_GATEWAY_DEPLOY_REMOTE_DIR="/opt/lerdr-gateway"
export LERDR_RELAY_ENV="$WORK_DIR/relay-deploy.env"
: > "$LERDR_RELAY_ENV"
run_deploy
...
grep -Fq 'mkdir -p /opt/lerdr-gateway && tar -xzf - -C /opt/lerdr-gateway' "$SSH_LOG" ||
    fail "deployment never copied the bundle"
grep -Fq 'cd /opt/lerdr-gateway && docker compose up -d --build --force-recreate gateway caddy' \
...
grep -Fq "LERDR_GATEWAY_URL='wss://gw.example.test,wss://community-a.example.test,wss://community-b.example.test'" \
    "$LERDR_RELAY_ENV" || fail "verified deployment did not record the ordered gateway list"
grep -Fq "LERDR_GATEWAY_SELECTION='ordered'" "$LERDR_RELAY_ENV" ||
...
export LERDR_GATEWAY_DEPLOY_DIR="$BUNDLE_DIR"
export LERDR_RELAY_ENV="$WORK_DIR/relay-unreachable.env"
: > "$LERDR_RELAY_ENV"
export LERDR_GATEWAY_DEPLOY_PUBLIC_TIMEOUT=0
run_deploy
unset LERDR_GATEWAY_DEPLOY_PUBLIC_TIMEOUT
...
EOF
chmod 700 "$STUB_DIR/curl"
export LERDR_RELAY_ENV="$WORK_DIR/relay-remembered.env"
: > "$LERDR_RELAY_ENV"
...
export LERDR_GATEWAY_DEPLOY_DIR="$REMEMBERED_BUNDLE"
export LERDR_GATEWAY_DEPLOY_SERVER="deploy@gw.example.test"
export LERDR_GATEWAY_DEPLOY_REMOTE_DIR="/srv/lerdr-gateway"
run_deploy
...
for EXPECTED in \
    "LERDR_GATEWAY_DEPLOY_HOST='gw.example.test'" \
    "LERDR_GATEWAY_DEPLOY_SERVER='deploy@gw.example.test'" \
    "LERDR_GATEWAY_DEPLOY_REMOTE_DIR='/srv/lerdr-gateway'" \
    "LERDR_GATEWAY_DEPLOY_EMAIL='ops@example.test'" \
    "LERDR_GATEWAY_DEPLOY_DIR='$REMEMBERED_BUNDLE'"; do
...
: > "$SSH_LOG"
rm -f "$STUB_STATE/started" "$STUB_STATE/uploaded.tar.gz"
unset LERDR_GATEWAY_DEPLOY_HOST LERDR_GATEWAY_DEPLOY_EMAIL \
    LERDR_GATEWAY_DEPLOY_DIR LERDR_GATEWAY_DEPLOY_SERVER \
...
grep -Fq 'mkdir -p /srv/lerdr-gateway && tar -xzf - -C /srv/lerdr-gateway' "$SSH_LOG" ||
...
grep -Fq "LERDR_GATEWAY_URL='wss://gw.example.test,wss://community-a.example.test,wss://community-b.example.test'" \
    "$LERDR_RELAY_ENV" || fail "remembered rerun did not record the verified gateway list"

# --- The final subscription choice can keep only the deployed gateway --------
: > "$SSH_LOG"
rm -f "$STUB_STATE/started" "$STUB_STATE/uploaded.tar.gz"
export LERDR_RELAY_ENV="$WORK_DIR/relay-custom-subscriptions.env"
: > "$LERDR_RELAY_ENV"
export LERDR_GATEWAY_SUBSCRIPTIONS="wss://gw.example.test"
run_deploy
[ "$STATUS" -eq 0 ] || fail "custom-subscription run exited $STATUS"
grep -Fq "LERDR_GATEWAY_URL='wss://gw.example.test'" "$LERDR_RELAY_ENV" ||
    fail "the explicit subscription list did not leave the deployed gateway alone"
if grep -Fq 'community-a.example.test' "$LERDR_RELAY_ENV"; then
...
unset LERDR_GATEWAY_SUBSCRIPTIONS
...
EOF
chmod 700 "$STUB_DIR/ssh"
export LERDR_GATEWAY_DEPLOY_SSH="ssh"
run_deploy
...
grep -Fq 'LERDR_GATEWAY_DEPLOY_SSH="ssh -i ~/.ssh/yourkey"' "$OUTPUT" ||
