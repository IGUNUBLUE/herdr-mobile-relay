...
INSTALLER=${LERDR_PLUGIN_INSTALLER:-${HERDR_PLUGIN_INSTALLER:-"$REPO_DIR/install.sh"}}
...
rollback_plugin_migration() {
    ...
    if [ -n "$PREVIOUS_RELEASE" ] && [ -d "$PREVIOUS_RELEASE" ]; then
        # current may still point at either side of the rename; call whichever
        # binary name the bundle it resolves to carries.
        activate_bin="$INSTALL_ROOT/current/lerdr"
        [ -x "$activate_bin" ] || activate_bin="$INSTALL_ROOT/current/herdr-mobile-relay"
        "$activate_bin" \
...
"$INSTALL_ROOT/current/lerdr" verify-release "$INSTALL_ROOT/current" >/dev/null
