...
BIN_LINK="${LERDR_RELAY_BIN_DIR:-${HERDR_RELAY_BIN_DIR:-$HOME/.local/bin}}/lerdr"
LEGACY_BIN_LINK="${LERDR_RELAY_BIN_DIR:-${HERDR_RELAY_BIN_DIR:-$HOME/.local/bin}}/herdr-mobile-relay"
...
safe_remove_bin_link() {
    ...
    # Its filename and executable bit alone are not ownership proof. Either side
    # of the rename counts: a pre-rename install left its binary behind.
    local active_binary
    for active_binary in "$RELEASE_ROOT/current/lerdr" "$LEGACY_RELEASE_ROOT/current/herdr-mobile-relay"; do
...
echo "  Binary link:  $BIN_LINK (and a pre-rename herdr-mobile-relay link)"
...
safe_remove_bin_link "$LEGACY_BIN_LINK"
